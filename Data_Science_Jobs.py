...
worktype_list = ['Part-time','Contract','Visa Sponsorship','Remote','Full-Time']
...
def kdnuggets_job_position(element):
    try: position = element.find("a").get_text().split(':')[1]
    except: position = element.find("a").get_text().split(':')[0]
    return position
...
def dataelixir_company_logo(element):
    try: logo = element.find("img").get('src')
    except: logo = 'NL'
    return logo
...
output_jsonload = json.loads(output_json)
